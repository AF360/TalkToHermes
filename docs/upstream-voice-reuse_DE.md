...
Die nachfolgend beschriebenen Implementierungsschnittstellen wurden im August 2026 gegen Hermes verifiziert. Vor Änderungen ist die aktuelle Upstream-Dokumentation erneut zu prüfen.
...
| OpenAI-kompatibles TTS | `tts.openai.base_url` | mit dediziertem `/audio/speech`-Adapter wiederverwendbar; `/synthesize` ist inkompatibel |
...
## Verifizierte API-Besonderheit

`/v1/runs` mit wiederholter `session_id` persistierte beim Zieltest vom 28.08.2026 Nachrichten, lud den vorherigen Turn jedoch nicht automatisch. `/api/sessions/{id}/chat/stream` lädt Historie, der erzeugte Run besitzt derzeit aber keine über `/v1/runs/{id}/approval` auflösbare Approval-Session. TalkToHermes liest deshalb vor jedem Run die kanonischen Hermes-Session-Nachrichten und übergibt sie als `conversation_history`. Ein Zwei-Turn-Marker-Test bestätigte Kontinuität mit vier persistierten Nachrichten; idempotentes Replay erzeugte keinen weiteren Turn.

## Provider-Herkunft aus dem Implementierungsreview

OmniVoice ist ein unabhängiger Clean-Room-Adapter zur Apache-2.0-API `k2-fsa/OmniVoice`; kein Code aus `scorbo2/ai-playground` wird kopiert oder benötigt. Der Adapter pinnt `omnivoice==0.2.1` und Modellrevision `c5fdb5ccb189668d56333f77ba2629f4cd7535f4`. Der Upstream-Wrapper liefert JSON/Base64 über `/synthesize`, nicht OpenAI-binär über `/audio/speech`, weshalb direkte Hermes-OpenAI-TTS-Konfiguration gegen den Legacy-Listener inkompatibel ist.
...
Der dedizierte STT-Service pinnt seinen Offline-`large-v3-turbo`-Snapshot und validiert Medien mit PyAV vor einer serialisierten CUDA/float16-Inferenz. Der unauthentifizierte Legacy-Listener auf Port `5005` wird absichtlich nicht wiederverwendet.

## Upgrade-Regel

Vor einer neuen Voice-Schnittstelle aktuelle offizielle Dokumentation und Upstream-Main prüfen. Ein wöchentlicher Read-only-Review meldet wesentliche Änderungen. Erhält der offizielle API Server stabile Audio-Endpunkte oder Hermes einen geeigneten iOS-Client, soll die entsprechende Eigenkomponente entfernt statt doppelt gepflegt werden.
