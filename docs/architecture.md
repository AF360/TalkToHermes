...
Tool activity is fail-closed per-call metadata. For every `tool.started` whose internal ID is present in the per-instance `exposed_tools` mapping, the bridge creates a distinct stable marker, translates the ID to a unique constrained alphanumeric display name, and records only the fixed `invoked` status, start time, and conservatively correlated approval/risk metadata. Runtime previews are always discarded and never persisted. An optional `tool_summaries` mapping supplies bounded, static, administrator-reviewed public descriptions keyed by exposed display name; REST and SSE add these labels only at egress, so historical event payloads cannot override them. Repeated calls are not deduplicated. Raw previews, arguments, results, paths, prompts, and unmapped tool IDs never cross the bridge boundary. Completion status and duration are deliberately absent because the current Hermes Runs stream provides no call ID with which to correlate parallel same-tool completions safely. The legacy unique `tools` list remains additive compatibility metadata for older apps.
